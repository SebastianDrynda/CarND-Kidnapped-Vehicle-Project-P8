...
void ParticleFilter::prediction(double delta_t, double std_pos[],
  ...
  for (auto &p : particles) {
    ...
    if (std::fabs(yaw_rate) > 0.0001) {
      ...
      p.x += v_yaw * (std::sin(p.theta + yaw_dt) - std::sin(p.theta));
      p.y += v_yaw * (-std::cos(p.theta + yaw_dt) + std::cos(p.theta));
      p.theta += yaw_dt;
    } else {
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted,
  ...
  for (auto &observation : observations) {
    // Set min distance as a really big number.
    ...
    for (auto lm_predicted : predicted) {
      ...
      double distance = dist(observation.x, observation.y, lm_predicted.x,
                             lm_predicted.y);
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    // Transform the observations to global (map) coordinate system
    vector<LandmarkObs> observations_global_coords;
    for (const auto &observation : observations) {
      double trans_x = cos(heading) * observation.x
          - sin(heading) * observation.y + px;
      double trans_y = sin(heading) * observation.x
          + cos(heading) * observation.y + py;
    ...
    for (auto &observation : observations_global_coords) {
      ...
      // Landmark's and observed X and Y coordinates
      double dy = y_lm - y_obs;

      // Calculating weight using a multivariate gaussian
      double dx = x_lm - x_obs;
      particles[i].weight *= (1 / (2 * M_PI * sx * sy))
          * std::exp(
              -(std::pow(dx, 2) / (2 * std::pow(sx, 2))
